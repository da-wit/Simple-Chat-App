...
export const sendMessage = async(req,res) => {
    try {
        ...
        let conversation = await Conversation.findOne({participantes:{$all: [senderId,receiverId]}})
        ...
        console.log("Error before the message controller",{error:error.message})
...
export const getMessages = async(req,res) => {
    try {
        ...
        const conversation = await Conversation.findOne({participantes:{$all:[senderId,userToChatId]}}).populate("messages");
        if(!conversation){
